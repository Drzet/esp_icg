...
#define LCD_BATCH_LINES 16
#define LCD_BATCH_PIXELS (ICG_LCD_WIDTH * LCD_BATCH_LINES)
...
static esp_err_t draw_block_sync(int y0, int y1, const uint16_t *pixels)
{
    if (xSemaphoreTake(s_lcd_mutex, portMAX_DELAY) != pdTRUE) return ESP_ERR_TIMEOUT;
    while (xSemaphoreTake(s_lcd_done, 0) == pdTRUE) {}

    esp_err_t ret = esp_lcd_panel_draw_bitmap(s_panel, 0, y0, ICG_LCD_WIDTH, y1, pixels);
...
static esp_err_t clear_screen(void)
{
    ...
    for (int y = 0; y < ICG_LCD_HEIGHT; ++y) {
        ESP_RETURN_ON_ERROR(draw_block_sync(y, y + 1, s_line), TAG, "clear line");
    }
    return ESP_OK;
}
...
esp_err_t display_init(void)
{
    ...
    spi_bus_config_t buscfg = {
        ...
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
        .max_transfer_sz = LCD_BATCH_PIXELS * 3,
    };
    ...
    /* Conversion buffer is in pixels; 16 scanlines greatly reduce SPI transaction overhead. */
    ESP_RETURN_ON_ERROR(esp_lcd_new_panel_ili9488_ips(io, &panel_cfg, LCD_BATCH_PIXELS, &s_panel),
                        TAG, "ili9488");
